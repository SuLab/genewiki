...
class PDB(object):
    ...
    def __init__(self, pdbid, hugosym,
                 pdbpath=None, pymolpath=None, commons=None):
        self.pdbid = pdbid
        ...
        if not os.path.exists(self.pdbpath):
            try: os.makedirs(self.pdbpath)
            except OSError:
                raise ValueError(
                    "Could not access pdb path at {} nor create it.".format(
                        self.pdbpath))
    ...
    def render(self, pdb_id=None, hugo_sym=None, pdb_file=None):
        ...
        if not pdb_id: pdb_id=self.pdbid
        if not hugo_sym: hugo_sym=self.hugosym

        # Attempt to download the pdb file if not explicitly passed
        if not pdb_file: pdb_file = self.pdbfile
        if not pdb_file: pdb_file = self.download(pdb_id)
        if not pdb_file: return None

        # Set up the future location of the image
        if pdb_id == None: return None
        png_file = '{pdbpath}Protein_{hugo_sym}_PDB_{pdb_id}.png'.format(
            pdbpath=self.pdbpath, hugo_sym=hugo_sym, pdb_id=pdb_id)

        # Launch pymol as a subprocess and wait for return
        rendercmd = "cmd.png('{png_file}', 1200, 1000)".format(
            png_file=png_file);
        pymolcmd = [self.pymolpath, '-c', pdb_file,
                    settings.pbbhome+'commands.pml', '-d', rendercmd]
        print( " ".join(pymolcmd) )
        try: subprocess.check_call(pymolcmd)
        except CalledProcessError as e:
            sys.stderr.write('pdb: error rendering pdb file for id {}'.format(
                    pdb_id))
            return None
    ...
    def uploadToCommons(self, description=None, png_file=None, commons=None):
        if not png_file: png_file=self.pngfile
        if not png_file: raise ValueError("No .png file specified.")

        if not commons: commons=self.commons
        if not commons:
